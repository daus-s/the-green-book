...
export default function Search({fields, table, name, JSX, onSelect}) {
    if (!JSX) {
    ...
    return (
        <div className="search" ref={ref}>
            <div className='search-container' style={isMobile?{width: 'calc(100% -20px)'}:{}}>
                <input className="search-bar" placeholder={`search ${name}...`} value={query} onChange={(e)=>setQuery(e.target.value)}/>
                <img style={{height: '32px'}} src="/search.png"/>
            </div>
            {visible?
                <div className="results-container" style={{width: 'calc(100% - 112px)'}}>
                    {results.map((result)=><JSX r={result} select={select} style={{maxWidth: '100%', paddingRight: '21px'}}/>)}
                </div>
            :
                <></>
            }
