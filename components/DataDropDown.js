...
export default function DataDropDown({ list, JSX, set, data }) {
                ...
                visible?
                    <div className="scroll-container" style={{top: `${height(width)}px`}}>
                        ...
                        list.map((item, index)=>
                            {
                                return (
                                    <JSX key={item?.index||item?.index===0?item.index:index} data={item} onClick={()=>leClick(item)} selected={(display?.index)?(display?.index)===(item?.index):display?.groupID?(display?.groupID)===(item?.groupID):false}/>
                                );
                            })
                        }
                    </div>
                ...
                visible?
                    <div className="scroll-container">
                        ...
                        list.map((item, index)=>
                            {
                                return (
                                    <DropDownItem key={item?.index||item?.index===0?item.index:index} data={item} onClick={()=>leClick(item)} selected={(display?.index)===(item?.index)}/>
                                );
                            })
                        }
                    </div>
