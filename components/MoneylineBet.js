...
const MoneyLineBet = ({ bet }) => {
    ...
    const handlePlaceBet = async (bet_amount, outcome) => {
        ...
        const { data, error } = await supabase.rpc("place_bet", {
            // python has the neww featurre like this wwhere is justt
            // def fun(_amount: int):
            //     ...
            //
            // usage:
    ...
    return (
        <div className="money-line bet" style={isMobile ? { width: elementWidth } : {}}>
            ...
            >
                <div className="it-hits">Hits {odds.hits ? odds.hits : NaN}</div>
                <div className="f-ck">Misses {odds.misses ? odds.misses : NaN}</div>
            </div>
            {result !== "Closed" && <MoneylinePlaceBetForm onSubmit={handlePlaceBet} bet={bet} />}
        </div>
    );
};
