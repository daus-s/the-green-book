    ...
    return (
        ...
        <div className="modal">
            ...
            <div className='request-text'>{approve?
                ...
                :
                <>Are you sure you want ignore <b>{guest?.username}</b>'s request to join {groupName?groupName:'this group'}?</>}</div>
            <div className="button-container">
            <button  className="cancel-button" onClick={onClose}>Cancel</button>
            <button className="confirm-button skinny" onClick={onConfirm}>{approve?'Accept':'Ignore'}</button>
            </div>
        </div>
        </Modal>
    );
