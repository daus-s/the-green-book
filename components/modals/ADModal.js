...
const ADModal = ({ isOpen, onClose, onConfirm, approve, guest, groupName}) => {
    ...
    return (
        ...
        <div className="modal">
            ...
            <div className='request-text'>{approve?<>Are you sure you want to let <b>{guest?.username}</b> join {groupName?groupName:'this group'}?</>:<>Are you sure you want ignore <b>{guest?.username}</b>'s request to join {groupName?groupName:'this group'}?</>}</div>
            <div className="button-container">
            <button  className="cancel-button" onClick={onClose}>Cancel</button>
            <button className="confirm-button skinny" onClick={onConfirm}>{approve?'Accept':'Reject'}</button>
            </div>
        </div>
        </Modal>
    );
