...
function LoginTree() {
    ...
    //add hover styling and login stuff
    return (
...
function UserTree() {
    ...
    return (
        <div className="tree-menu unloaded" style={isMobile ? { width: elementWidth, margin: landscape ? "22px" : "92px 52px 52px 52px" } : {}}>
            <Tree />
            {leaves.map((content, index) => (
                <Leaf content={content} key={index} fraction={{ num: index, den: leaves.length }} />
            ))}
        </div>
    );
}
...
function Leaf({ fraction, content }) {
    ...
    if (!fraction || (!fraction.num && typeof fraction.num !== "number") || (!fraction.den && typeof fraction.den !== "number")) {
        //type checking
