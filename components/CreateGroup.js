    ...
    const addUser = async (u) => {
        ...
        if (user) {
            ...
            for (let i = 0; i < length; ++i) {
                //does
                if (users[i]) {
                    contains ||= users[i].id === user.id && users[i].name === user.name && users[i].username === user.username; //uniqeness contrainst only applies on these fields this is enough to verify the closeness of user
                }
            }
            if (!contains) {
        ...
        <div className="create-group page" style={isMobile ? mobileStyle.form : {}}>
            <div className="new-group" style={isMobile ? mobileStyle.form : {}}>
                ...
                <div>
                    ...
                    <input type="text" value={name} onChange={(e) => handleNameChange(e)} required style={{ fontSize: "inherit", width: "100%", margin: "10px 0px 10px 0px" }} />
                    <FindUserList users={users} addUser={addUser} create={true} remove={remove} />
                    <div className="btn-cntr">
                        <button className="create-group submit" onClick={handleSubmit}>
                            Create
                        </button>
                    </div>
                </div>
